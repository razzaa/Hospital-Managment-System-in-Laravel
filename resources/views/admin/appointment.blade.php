                        ...
                        <div class="card-body">
                            <form name="my-form"  method="PUT" action="{{ route('admin_appointment_show.create') }}">
                                ...
                                <option value="0" selected disabled> -- Select One --</option>
                                    ...
                                    <option value="{{$patient->name}} ({{ $patient->email }})"  {{ (isset($doctor->id) || old('id')) }}>({{$patient->id}}) {{ $patient->name }} </option>
                                    @endforeach 
                               </select>
                               </div>
                                </div>
...
<table class="table table-bordered table-hover">
  ...
  @foreach($appointment as $data)
    <tr>
      ...
      <td>{{link_to_route('admin_appointment_show.edit','',[$data->id],['class'=>'fas fa-edit btn btn-success'])}}</td>
                       <td>
